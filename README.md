...
- Doc-wide ID registry — built once per document via `adoc::ast::IdRegistry::collect(&doc)`. Picks up section IDs, block IDs from `[#…]` shorthand, inline anchors (`anchor:id[]`), bibliography anchors (`[[[id]]]`), and `<a id="…">` emitted from `Inline::RawHtml`. The HTML5 converter validates every `<<…>>` / `xref:…[]` target against the registry and emits `tracing::warn!("dangling xref", target = …)` warnings for unresolved references — run with `-v` (or `RUST_LOG=warn`) to see them. The HTML still emits the dangling href.
